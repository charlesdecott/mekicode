...
| Lignes | Zone | Contenu |
|---|---|---|
| 1–11 | Docstring & imports | json, datetime, yaml ; `MODEL/ROOT/STATE_DIR/client/paint` de core, `register_tool` de tools |
...
- Modules liés : [[core-py]] (client, chemins, `paint`), [[tools-py]] (`register_tool`), [[main-py]] (assemblage du prompt et déclenchement de la compaction), [[loop-py]] (expose `load_skill` au modèle via le registre)
