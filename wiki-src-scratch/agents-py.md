...
| 8–18 | Imports | `paint`, `agent_loop`, mailbox, tasks (claim/complete/fail), `register_tool` |
| 21–23 | Helper | `_text_of()` — extraire le texte d'une réponse API |
| 26–55 | Subagent (s04) | `SUBAGENT_SYSTEM`, `spawn_subagent`, enregistrement de l'outil `subagent` |
| 58–79 | Équipe — données | `TEAMMATES`, `_ACTIVE_TEAM`, `AgentState` (FSM s10) |
| 82–136 | Équipe — classe | `Team` : start / stop / status / `_teammate_loop` |
| 139–179 | Équipe — côté lead | `send_to_teammate` (corrélation `req_id`), `_register_team_tools` |
| 182–217 | Workers (s11) | `run_autonomous_agent`, `start_workers` |

## Constantes et configuration

- **`SUBAGENT_SYSTEM` (lignes 28–31)** : le system prompt des subagents — « working on a specific subtask at `os.getcwd()` … Summarize your result clearly at the end ». La consigne de résumé final n'est pas décorative : c'est ce résumé qui constitue la valeur de retour de l'outil.
- **`TEAMMATES` (lignes 60–71)** : les deux spécialistes repris des prompts de la source — `explorer` (compréhension de code : bash, read, glob, grep) et `writer` (création/édition : write, read, bash). Un dict nom → system prompt ; `Team` accepte n'importe quel dict de même forme.
- **`_ACTIVE_TEAM` (ligne 73)** : global de module désignant l'équipe visée par `send_to_teammate` — posé par `Team.start()`, effacé par `Team.stop()`. Une seule équipe active à la fois.

## Les fonctions, une à une

### `_text_of(message)` — lignes 21–23
...
Le pont entre une réponse API structurée (liste de blocs) et la chaîne qu'attendent les canaux inter-agents. Utilisé trois fois : retour du subagent, réponse d'équipier, résultat de tâche d'un worker. Le test `hasattr(b, "text")` écarte les blocs `tool_use` résiduels sans dépendre du type exact des objets du SDK.

### `spawn_subagent(task, system=None, tools=None)` — lignes 34–41
...
### `Team.__init__(teammates=None)` — lignes 85–90
...
### `Team.start(mailbox=None)` — lignes 92–103
...
### `Team._teammate_loop(name, prompt)` — lignes 120–136
...
### `send_to_teammate(name, message, timeout=120)` — lignes 139–158
...
- **Gardes d'entrée (lignes 141–145)** : pas d'équipe active ou destinataire inconnu → message d'erreur textuel (donnée pour le modèle, pas exception).
- **Corrélation (lignes 146–153)** : un `req_id` de 8 hexas par requête ; seule la réponse portant **ce** `req_id` est acceptée. Toute autre réponse trouvée dans la boîte du lead est signalée et jetée.
- **Timeout (ligne 158)** : au-delà de 120 s, retour explicite — et grâce au `req_id`, la réponse qui arriverait après coup sera reconnue comme étrangère par l'appel suivant au lieu de lui être attribuée.

### `_register_team_tools(teammates)` — lignes 161–179
...
### `run_autonomous_agent(name, max_idle=3)` — lignes 184–207
...
### `start_workers(n=2)` — lignes 210–217
...
- **Effet de bord à l'import** : le `register_tool` des lignes 44–55 s'exécute quand le module est importé — importer `agents` modifie les registres globaux de [[tools-py]]. Voulu (c'est le mécanisme d'extension du harness), mais à savoir : pas d'outil `subagent` tant que personne n'importe `agents`.
...
- Modules liés : [[mailbox-py]] (le canal de l'équipe), [[loop-py]] (la boucle unique que tout agent délégué exécute), [[tasks-py]] (`claim_next_task` / `complete_task` / `fail_task` pour les workers), [[tools-py]] (`register_tool` des trois outils), [[main-py]] (commandes `:team` et `:workers`)
