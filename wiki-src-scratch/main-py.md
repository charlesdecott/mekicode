...
| 50–61 | Helpers | `_text_of()`, `_repair()` |
| 64–116 | Commandes | `handle_command()` — le dispatcher des `:` |
| 119–163 | Boucle | `repl()` — saisie, tour d'agent, compaction, auto-save |
| 166–181 | CLI | `main()` — argparse + `asyncio.run` |
| 184–185 | Entrée | `if __name__ == "__main__"` |
...
### `_text_of(message)` — lignes 50–51

Concatène les blocs texte d'une réponse SDK (`hasattr(b, "text")`) — utilisé pour le payload de l'événement `assistant_message`.

### `_repair(messages)` — lignes 54–61

Le filet de sécurité après une interruption ou une erreur en plein tour : si le dernier message est un tour assistant contenant des `tool_use` **sans** les `tool_result` correspondants, l'API refuserait tout l'historique au tour suivant. `_repair` retire ce message orphelin. Le test couvre les deux formes possibles d'un bloc — objet SDK (`getattr(b, "type", None)`) ou dict pur issu d'une session rechargée (`b.get("type")`), lignes 58–59.

### `handle_command(cmd, state, args)` — lignes 64–116

Le dispatcher des commandes `:`. `state` est le dict partagé `{messages, sid, team}` ; `cmd` est découpé par `partition(" ")` en verbe + reste. Toute commande inconnue ou incomplète tombe sur le message jaune final (ligne 116) avec rappel de `:help`.

| Commande | Lignes | Effet | Module sollicité |
|---|---|---|---|
| `:help` | 68–69 | affiche `HELP` | — |
| `:sessions` | 70–71 | liste les sessions sauvegardées | [[sessions-py]] |
| `:resume <id>` | 72–76 | recharge l'historique et le sid | [[sessions-py]] |
| `:fork <id>` | 72–76 | duplique puis recharge le fork | [[sessions-py]] |
| `:title <texte>` | 77–79 | (re)nomme la session courante | [[sessions-py]] |
| `:save` | 80–82 | sauvegarde manuelle | [[sessions-py]] |
| `:todos` | 83–84 | plan courant (todo_read) | [[tasks-py]] |
| `:tasks` | 85–86 | graphe de tâches persistant | [[tasks-py]] |
| `:requeue` | 87–88 | repasse les tâches failed en pending | [[tasks-py]] |
| `:team on\|off\|status` | 89–97 | équipiers persistants (explorer, writer) | [[agents-py]] + [[mailbox-py]] |
| `:workers <n>` | 98–101 | n workers autonomes sur le board (défaut 2) | [[agents-py]] |
| `:wt <t1> \| <t2> ...` | 102–106 | une tâche par worktree git isolé | [[worktree-py]] |
| `:compact` | 107–108 | compaction forcée du contexte | [[context-py]] |
| `:cache` | 109–110 | stats du prompt caching | [[loop-py]] |
| `:mcp` | 111–114 | démarrage paresseux + état des serveurs | [[mcp-runtime-py]] |
| `:quit` / `:q` / `:exit` | (dans `repl`, ligne 136) | quitter | — |

Trois détails notables :

- **`:resume` et `:fork` partagent une branche** (lignes 72–76) : fork = `fork_session(rest)` puis chargement du nouvel id ; resume = chargement direct. Dans les deux cas `state["messages"]` et `state["sid"]` sont remplacés d'un coup.
- **`:team on`** (lignes 90–92) construit la `Team` avec la mailbox du backend choisi en CLI : `state["team"].start(get_mailbox(args.backend))`. Tout autre argument (`status`, vide…) affiche `status()` ou un rappel.
- **`:wt`** (lignes 102–106) découpe sur `|`, puis `await asyncio.to_thread(worktree.run_parallel_tasks, jobs)` — les threads et leurs `join` bloquants ne doivent pas geler l'event loop.

### `repl(args)` — lignes 119–163
...
- **Saisie** (ligne 130) : `await asyncio.to_thread(input, ...)` — l'`input` bloquant tourne dans un thread ; `EOFError`/`KeyboardInterrupt` sur la saisie = sortie propre de la boucle.
- **Tour d'agent** (lignes 143–146) : `agent_loop_async` de [[loop-py]] avec `parallel` et `cache` pilotés par les flags. Un double Ctrl+C (le `KeyboardInterrupt` de `Interrupts`) ou une exception quelconque abandonne le tour mais **répare l'historique** (`_repair`) — la conversation continue.
- **Après chaque tour** (lignes 153–154) : `maybe_compact` (la coupe propre de [[context-py]]) puis auto-save — l'affectation par tranche `state["messages"][:] = ...` mute la liste en place, toutes les références restent valides.
- **`finally`** (lignes 155–163) : arrêt de l'équipe, `await mcp_runtime.stop_mcp()` (même tâche asyncio que le démarrage — la contrainte anyio de [[mcp-runtime-py]]), `emit("session_end")`, résumé du cache si utilisé, et le rappel `:resume <sid>` pour la prochaine fois.
...
Une incompatibilité est arbitrée à la main (lignes 174–177) :
...
- **`state["messages"][:] = ...`** (lignes 108 et 153) : affectation par tranche, pas réaffectation — la compaction remplace le *contenu* de la liste que d'autres closures pourraient référencer.
...
- Modules liés : [[loop-py]] (la boucle qu'il pilote), [[core-py]] (system, events, couleurs), [[context-py]], [[sessions-py]], [[tasks-py]], [[agents-py]], [[mailbox-py]], [[worktree-py]], [[mcp-runtime-py]], [[tools-py]] (indirect : le registre que la boucle consomme)
