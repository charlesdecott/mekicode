...
| 8–13 | Imports | stdlib + `STATE_DIR`, `paint` ([[core-py]]) |
| 15–18 | Constantes | `SESSIONS_DIR` (mkdir à l'import), `_META_KEYS` |
| 21–44 | Helpers privés | `_now`, `_new_id`, `_path`, `_read`, `_write` |
| 47–56 | Sérialisation | `_serialize` — le pivot du module |
| 59–79 | API | `save_session` |
| 82–104 | API | `load_session`, `list_sessions` |
| 107–121 | API | `fork_session`, `set_title` |
...
### `_write(data)` — lignes 41–44

Le point de sortie unique : rafraîchit `data["updated"]` puis écrit en `indent=2, ensure_ascii=False` (accents lisibles dans le fichier), encodage UTF-8 explicite. Toute écriture passant par là, `updated` ne peut jamais être oublié.

### `_serialize(messages)` — lignes 47–56
...
### `save_session(messages, sid=None, title=None)` — lignes 59–79
...
- **Lignes 62–66** : si le fichier existe, on le relit pour préserver `created` et `title` ; s'il est corrompu, warning jaune et réécriture propre — la sauvegarde n'échoue jamais à cause d'un fichier malade.
- **Ligne 67** : `sid = sid or _new_id()` — première sauvegarde = création.
- **Lignes 72–75** : auto-titrage comme s17 — faute de titre, le premier message user dont le `content` est une *string* fournit les 50 premiers caractères (repli `"Session"`). Les messages user à contenu liste (les `tool_result`) sont ignorés par le `isinstance(m.get("content"), str)`.
- **Lignes 76–78** : `turns = len(messages)`, sérialisation, `_write` (qui pose `updated`).

### `load_session(sid)` — lignes 82–85
...
Un JSON corrompu est signalé et sauté, le listing continue. L'id est peint en cyan (ligne 101) ; titre tronqué à 40 caractères, date à 16 (`YYYY-MM-DDTHH:MM`). Vide → `"(aucune session)"`, jamais une chaîne vide ambiguë.

### `fork_session(sid)` — lignes 107–114
...
### `set_title(sid, title)` — lignes 117–121
...
- **Pas de verrou ni d'écriture atomique** : deux REPL ouverts sur le même sid s'écrasent mutuellement (dernier écrivain gagne), et un crash en plein `write_text` peut laisser un JSON tronqué — que `save_session` et `list_sessions` savent toutefois absorber sans crash.
- **L'auto-titre est figé à la première sauvegarde** : `if not data.get("title")` (ligne 72) ne recalcule jamais — renommer passe par `:title`. Après une compaction de [[context-py]], le premier message user peut être le résumé : seul le titre d'une session *nouvelle* sauvegardée à ce moment-là en hériterait.
...
- Modules liés : [[core-py]] (`STATE_DIR`, `paint`), [[main-py]] (auto-save et commandes `:`), [[context-py]] (la compaction — l'autre mécanisme de survie de l'historique, dans la fenêtre du modèle plutôt que sur disque)
