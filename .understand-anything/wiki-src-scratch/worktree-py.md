...
| 15–24 | Imports | stdlib + `paint`/`text_of` ([[core-py]]) et `agent_loop` ([[loop-py]]) |
...
- **Ligne 129** : `res["result"] = text_of(final)` — le texte de la réponse est extrait par `text_of` de [[core-py]] (concaténation des blocs texte, `tool_use` ignoré), au lieu d'une concaténation inline.
...
- Modules liés : [[loop-py]] (la façade sync `agent_loop` exécutée dans chaque worktree), [[core-py]] (`paint`, `text_of` — import ligne 23), [[main-py]] (commande `:wt`), [[agents-py]] (l'autre modèle de parallélisme — workers sans isolation, sur l'arbre partagé)
