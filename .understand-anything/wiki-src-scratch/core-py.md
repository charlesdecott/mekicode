...
> **En une phrase** : tout ce que les dix autres modules partagent — client Anthropic, chemins (`ROOT`, `STATE_DIR`), `config.yaml` mis en cache, gouvernance trois tiers (s15), event bus à veto (s16), couleurs ANSI et trois utilitaires de mutualisation (`text_of`, `drain_queue`, `write_json`) — chargé une fois, à l'import.
...
| 6–11 | Imports stdlib | `json`, `os`, `queue`, `re`, `sys`, `Path` |
| 13–21 | Encodage console | FIX : stdout/stderr/stdin reconfigurés en UTF-8 (Windows cp1252) |
| 23–36 | Confort terminal | `readline` et `colorama` optionnels, comme la source |
| 38–45 | Env & client | `load_dotenv(override=True)`, purge du token si gateway tiers |
| 47–58 | Constantes | `client`, `MODEL`, `DEFAULT_SYSTEM`, `ROOT`, `STATE_DIR`, `CONFIG_PATH` |
| 60–65 | Couleurs | `_ANSI` + `paint()` |
| 68–86 | Config | `_CONFIG` (cache module), `load_config()`, `load_rules()` |
| 89–109 | Gouvernance | `check_permission()` — trois tiers deny/allow/ask (s15) |
| 112–136 | Event bus | `HOOKS`, `on()`, `emit()` (s16) |
| 139–158 | Utilitaires partagés | `text_of()`, `drain_queue()`, `write_json()` — mutualisations transversales |

## Constantes et configuration

- **`client` (ligne 47)** : `Anthropic(base_url=os.getenv("ANTHROPIC_BASE_URL"))` — un seul client pour tout le harness. Si `ANTHROPIC_BASE_URL` est défini, les lignes 43–45 retirent `ANTHROPIC_AUTH_TOKEN` de l'environnement : on n'envoie pas un token résiduel vers un gateway tiers.
- **`MODEL` (ligne 48)** : env `MODEL_ID`, défaut `"claude-sonnet-4-6"`.
- **`DEFAULT_SYSTEM` (lignes 49–51)** : « You are a coding agent at `<cwd>`… » — `os.getcwd()` est capturé **à l'import** ; lancer le harness depuis un autre répertoire change le prompt.
- **`ROOT` (ligne 53)** : `Path(__file__).parent` — l'ancre de tous les chemins du projet (jamais `parent.parent`, voir Bugs ci-dessous).
- **`STATE_DIR` (lignes 54–55)** : `ROOT/".state"` (surchargeable via env `MEKI_STATE_DIR`), `mkdir` immédiat. Tout l'état runtime ([[tasks-py]], [[mailbox-py]], [[sessions-py]], mémoire de [[context-py]]) vit dessous.
- **`CONFIG_PATH` (ligne 58)** : `ROOT / "config.yaml"`.
- **`_ANSI` (ligne 60)** : six couleurs (red/green/yellow/cyan/magenta/dim) pour `paint()`.
- **`HOOKS` (ligne 115)** : dict `event → liste de hooks`, le registre de l'event bus. Événements émis par le harness : `session_start`, `user_message`, `pre_tool`, `post_tool`, `assistant_message`, `session_end`.
...
### `load_config()` — lignes 71–80

Charge `config.yaml` complet et le met en cache module (`_CONFIG`, ligne 68) : le fichier n'est lu **qu'une seule fois** par processus.
...
- **Ligne 77** : `or {}` — un yaml vide donne `None` chez `yaml.safe_load` ; on normalise en dict.
- **Lignes 78–79** : fichier absent → dict vide, pas de crash. Le harness tourne sans config (mais alors sans règles — tout est « allowed by default »).

### `load_rules()` — lignes 83–86
...
### `check_permission(tool_name, input_str, rules=None)` — lignes 89–109
...
- **`ask_user` (lignes 100–108)** : affiche outil + arguments tronqués à 100 caractères + raison, puis `input("  Autoriser ? [y/N] ")`. `EOFError`/`KeyboardInterrupt` ⇒ réponse `"n"` : **en cas de doute, on refuse**. Seuls `y`/`yes` autorisent.
- **Ligne 109** : aucun tier ne matche → `(True, "allowed by default (no rule matched)")`. Politique permissive par défaut — la sécurité réelle vient des règles `always_deny` du yaml et de la `_ALWAYS_BLOCK` de [[tools-py]].
...
### `emit(event, payload)` — lignes 123–136
...
- **`is False` (ligne 132)** : seul un `False` explicite vaut veto — un hook qui renvoie `None` (cas du hook « observateur » qui ne return rien) n'a aucun effet sur le verdict.
...
### `text_of(message)` — lignes 141–143
...
Une réponse de l'API est une liste de blocs hétérogènes (`text`, `tool_use`, …) ; `text_of` ne garde que le texte. Le `getattr(b, "text", "")` ignore proprement tout bloc sans attribut `.text` (chaîne vide). Cette implémentation vivait dupliquée dans quatre modules ([[agents-py]], [[main-py]], [[context-py]], [[worktree-py]]) — elle est désormais mutualisée ici.

### `drain_queue(q)` — lignes 146–153
...
Boucle de drainage non bloquante : `get_nowait()` jusqu'à `queue.Empty`, qui sert de condition d'arrêt et de sortie. Mutualise les boucles de [[tools-py]] (`drain_notifications`) et de [[loop-py]] (`Interrupts.drain`).

### `write_json(path, data)` — lignes 156–158
...
Le format unique de persistance JSON du harness : `indent=2` (lisible) et `ensure_ascii=False` (les accents français restent en clair). Mutualise la sérialisation de [[tasks-py]] (`_save`) et de [[sessions-py]] (`_write`).

## Bugs de la source corrigés ici

- **`CONFIG_PATH` ancré sur `ROOT` (lignes 56–58)** — dans le core de la source (utilisé par s15), `_PERM_CONFIG` était construit avec `Path(__file__).parent.parent`, soit **hors du repo** : le yaml n'était jamais trouvé, `load_rules()` renvoyait des listes vides et toute la gouvernance tournait à blanc, silencieusement. Correction : `CONFIG_PATH = ROOT / "config.yaml"` (et la règle de la spec : jamais `parent.parent`).
- **`emit()` ne s'avale plus la langue (lignes 126–135)** — en s16, le `except Exception: pass` autour de l'appel des hooks neutralisait sans aucune trace un hook de sécurité buggé. Correction : l'exception est affichée en rouge avec le nom de l'événement. (Le choix de ne pas transformer l'exception en veto est conservé — voir Pièges.)
- **Console Windows UTF-8 (lignes 13–21)** — durcissement propre à mekicode (marqué `# FIX(mekicode):` mais sans équivalent dans la source) : en cp1252, les caractères `→` ou `≈` des affichages (stats de cache de [[loop-py]] notamment) faisaient crasher `print`, et un pipe UTF-8 vers stdin était décodé de travers. Les trois flux sont reconfigurés en `utf-8/replace` **avant** le wrap colorama.
...
- [[tools-py]] — `paint`, `drain_queue`
- [[loop-py]] — `client`, `MODEL`, `DEFAULT_SYSTEM`, `check_permission`, `emit`, `paint`, `drain_queue`
- [[context-py]] — `MODEL`, `ROOT`, `STATE_DIR`, `client`, `paint`, `text_of`
- [[tasks-py]] — `STATE_DIR`, `paint`, `write_json`
- [[mailbox-py]] — `STATE_DIR`, `paint`
- [[agents-py]] — `paint`, `text_of`
- [[worktree-py]] — `paint`, `text_of`
- [[sessions-py]] — `STATE_DIR`, `paint`, `write_json`
- [[mcp-runtime-py]] — `load_config`, `paint`
- [[main-py]] — `DEFAULT_SYSTEM`, `emit`, `paint`, `text_of`

Les trois utilitaires partagés ajoutés au socle :

- **`text_of`** — utilisé par [[agents-py]], [[main-py]], [[context-py]] et [[worktree-py]] (extraction du texte d'une réponse API).
- **`write_json`** — utilisé par [[tasks-py]] et [[sessions-py]] (persistance JSON indentée UTF-8).
- **`drain_queue`** — utilisé par [[tools-py]] et [[loop-py]] (vidage non bloquant d'une file).
