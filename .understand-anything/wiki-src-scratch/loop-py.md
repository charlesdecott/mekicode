...
| 8–15 | Imports | stdlib + [[core-py]] (client, gardes, `drain_queue`) + [[tools-py]] (palette) |
| 18–48 | Cache (s20) | `CacheStats` + instance globale `CACHE` |
| 51–85 | Interruptions (s19) | `Interrupts` + instance globale `INTERRUPTS` |
| 88–112 | Gardes communes | `_precheck` (hook + permission), `_finish` (post_tool + tool_result) |
| 115–161 | Dispatch | `dispatch_tools_async` (gather), `dispatch_tools` (séquentiel) |
| 164–195 | Appel API | `stream_turn` — streaming + stratégie de cache s20 |
| 198–214 | Injection user | `_inject_user` (FIX s19) |
| 217–242 | La boucle | `agent_loop_async` + façade `agent_loop` |

## Constantes et configuration

- **`CACHE` (ligne 48)** : instance globale de `CacheStats`, alimentée par `stream_turn`, lue par la commande `:cache` de [[main-py]].
- **`INTERRUPTS` (ligne 85)** : instance globale d'`Interrupts` ; le handler SIGINT n'est posé que si on appelle `INTERRUPTS.install()` (fait par [[main-py]] au boot).
...
- **`install()` (lignes 63–65)** : à appeler une fois, depuis le **thread principal** (contrainte de `signal.signal`).
- **`drain()` (lignes 80–82)** : vidage non bloquant de la file, désormais une ligne — `return drain_queue(self._queue)`. La boucle `while True / try get_nowait / except _queue.Empty` est mutualisée dans [[core-py]] (`drain_queue`, importé en tête), partagée avec `drain_notifications` de [[tools-py]] qui draine la même façon sa propre file.

### `_precheck(block, permissions, hooks)` — lignes 90–104
...
- **Ligne 93** : `first` = la **première valeur** du dict input (`next(iter(...), "")`, robuste au dict vide) — pour `bash` c'est la commande, pour `read`/`write` le chemin. Elle sert à la fois à l'affichage jaune (tronquée à 80 caractères) et au jugement de permission.
- **Ordre : hook avant permission** — un veto de hook économise même le prompt `ask_user`.
- **Ligne 101** : `check_permission` reçoit `first`, la valeur nue — exactement la sémantique de s15. Les motifs ancrés du `config.yaml` (`^ls( |$)`, `^rm `, `^git (commit|…)`) matchent donc pleinement (voir Bugs : notre première intégration passait la repr du dict).
- **Ligne 103** : normalisation du refus — `check_permission` renvoie déjà `"Denied: …"` pour un deny, mais « user decision » (refus au prompt) est réhabillé en `"Denied: user decision"`. Le modèle reçoit toujours un message homogène.

### `_finish(block, output, hooks)` — lignes 107–112
...
### `dispatch_tools_async(content, dispatch=None, permissions=True, hooks=True)` — lignes 115–142

L'exécuteur parallèle de s18, en trois temps :

1. **Gardes séquentielles** (lignes 124–129) : `_precheck` sur chaque bloc, dans l'ordre — les prompts `ask_user` sont interactifs et ne se parallélisent pas. Les bloqués reçoivent leur tool_result d'office, les autres vont dans `runnable`.
2. **Exécution concurrente** (lignes 131–141) :
...
3. **Finalisation ordonnée** (ligne 142) : `_finish` dans l'ordre **original** des blocs (`for b in blocks`), pas dans l'ordre d'achèvement — l'affichage et la liste de tool_results restent déterministes.

### `dispatch_tools(content, dispatch=None, permissions=True, hooks=True)` — lignes 145–161
...
### `stream_turn(messages, tools, system, cache=True, extra_kwargs=None)` — lignes 166–195
...
- **`copy.deepcopy(tools)` (ligne 180)** : décisif. `tools` est par défaut LA liste partagée de [[tools-py]], maintenue en place par `register_tool` — y poser `cache_control` directement la muterait pour tout le monde (et le marqueur s'empilerait à chaque tour). On marque une copie, jamais l'objet partagé.
...
- **Lignes 193–194** : `CACHE.record(response.usage)` seulement si `cache=True` — désactiver le cache désactive aussi la comptabilité.

### `_inject_user(messages, texts)` — lignes 200–214
...
### `agent_loop_async(messages, tools=None, dispatch=None, system=None, parallel=True, cache=True)` — lignes 217–237
...
- **Tête de tour (lignes 227–228)** : les notifications de fond (s08, préfixées `[notification]`) et les interruptions (s19, déjà préfixées `[INTERRUPT]`) sont drainées et injectées comme contenu user — le modèle les voit au prochain appel, dans le fil normal de la conversation.
- **Défauts (lignes 222–225)** : `tools if tools is not None else TOOLS` — le test `is not None` (et non la truthiness) permet de passer explicitement `[]` pour une boucle sans outils. `dispatch` par défaut suit `parallel` : `ASYNC_DISPATCH` ou `DISPATCH` de [[tools-py]].
...
### `agent_loop(messages, **kw)` — lignes 240–242
...
- Modules liés : [[core-py]] (client, `check_permission`, `drain_queue`, `emit`, `paint`), [[tools-py]] (palette par défaut, notifications de fond), [[agents-py]] / [[worktree-py]] (consommateurs de la façade sync), [[main-py]] (consommateur async + Ctrl+C), [[sessions-py]] (sérialise l'historique muté), [[context-py]] (compacte ce même historique)
