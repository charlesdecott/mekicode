...
Délègue la sérialisation à `write_json` de [[core-py]] (`json.dumps(indent=2, ensure_ascii=False)`, UTF-8) — le format unique de persistance du harness, mutualisé plutôt que recopié ici. JSON lisible par un humain, accents en clair. Pas d'écriture atomique (pas de fichier temporaire + rename) : la protection contre la corruption est du côté lecture, dans `_load`.
...
- Modules liés : [[core-py]] (`STATE_DIR`, `paint`, `write_json` — import ligne 8), [[tools-py]] (`register_tool`), [[agents-py]] (workers consommateurs du board), [[main-py]] (commandes `:todos`, `:tasks`, `:requeue`), [[loop-py]] (sert les outils au modèle)
